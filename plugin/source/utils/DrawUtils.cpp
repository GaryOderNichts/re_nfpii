    ...
    for (; *string; string++) {
        ...
        if (sft_lookup(&pFont, *string, &gid) >= 0) {
            ...
            if (sft_gmetrics(&pFont, gid, &mtx) < 0) {
                DEBUG_FUNCTION_LINE("Failed to get glyph metrics");
                return;
            }
            ...
            if (!buffer) {
                DEBUG_FUNCTION_LINE("Failed to allocate memory for glyph");
                return;
            }
            img.pixels = buffer.get();
            if (sft_render(&pFont, gid, img) < 0) {
                DEBUG_FUNCTION_LINE("Failed to render glyph");
                return;
            } else {
...
uint32_t DrawUtils::getTextWidth(const wchar_t *string) {
    ...
    for (; *string; string++) {
        ...
        if (sft_lookup(&pFont, *string, &gid) >= 0) {
            SFT_GMetrics mtx;
            if (sft_gmetrics(&pFont, gid, &mtx) < 0) {
                DEBUG_FUNCTION_LINE("bad glyph metrics");
            }
